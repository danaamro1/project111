...
class MyHTTPRequestHandler(BaseHTTPRequestHandler):
    ...
    def authenticate(self):
        ...
        if auth_header:
            # يفصل الرأس عن البيانات
            auth_data = auth_header.split(' ')[1]
            # يقوم بفك تشفير البيانات بناءً على base64 ويحولها إلى string
            decoded_auth_data = base64.b64decode(auth_data).decode('utf-8')
            # يفصل البيانات الفكرية
...
def run(server_class=HTTPServer, handler_class=MyHTTPRequestHandler, port=7000):
    server_address = ('', port)
